...
void TForm8::LoadClients() {
	...
	ADOQuery->SQL->Text = "SELECT * FROM Клиенты";
    ADOQuery->Open();
    while (!ADOQuery->Eof) {
...
void __fastcall TForm8::ListBoxWorkersClick(TObject *Sender) {
    if (ListBoxWorkers->ItemIndex != -1) {
		ADOQuery->SQL->Text = "SELECT * FROM Клиенты WHERE ФИО = :Name";
...
void __fastcall TForm8::ButtonAddClick(TObject *Sender)
{
    if (ListBoxWorkers->ItemIndex != -1) {
		try {
			ADOQuery->SQL->Text = "UPDATE Клиенты SET ФИО = :Name WHERE ФИО = :OldName";
